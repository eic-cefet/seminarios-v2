...
@if($count === 1)
Não esqueça! Você está inscrito {{ $singleSeminar->ifMasculine('no', 'na') }} {{ $singleSeminar->inlineName() }} que acontecerá **amanhã**:
@else
Não esqueça! Você está inscrito {{ $collectionDescriptor->ifMasculine('nos', 'nas') }} {{ $collectionDescriptor->noun }} que acontecerão **amanhã**:
@endif
