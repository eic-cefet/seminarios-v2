...
describe('SubjectSeminars', () => {
    ...
    it('shows not-found state when subject is null after loading', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByRole('heading', { name: /tópico não encontrado/i })).toBeInTheDocument();
        });

        const link = screen.getByRole('link', { name: /voltar para tópicos/i });
        expect(link).toHaveAttribute('href', '/topicos');
    });
});
