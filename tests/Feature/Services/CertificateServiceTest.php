...
it('does not mark a failed upload as an existing certificate', function (string $format) {
    ...
    $disk->shouldReceive('put')->with('certificates/failed-upload.'.$format, Mockery::type('string'), 'private')->andReturn(false);
...
it('does not report a certificate as present when legacy migration fails', function () {
    $registration = Registration::factory()->create(['certificate_code' => 'failed-move']);
    $legacyPath = 'certificates/'.$registration->seminar->scheduled_at->year.'/'.$registration->seminar->slug.'/failed-move.jpg';
    Storage::disk('s3')->put($legacyPath, 'original');
    $disk = Mockery::mock(Storage::disk('s3'))->makePartial();
    $disk->shouldReceive('move')->with($legacyPath, 'certificates/failed-move.jpg')->andReturn(false);
    Storage::shouldReceive('disk')->with('s3')->andReturn($disk);

    expect(fn () => (new CertificateService)->jpgExists($registration))
        ->toThrow(UnableToMoveFile::class);
    expect(Cache::get('certificate_exists_jpg:failed-move'))->not->toBeTrue();
});
