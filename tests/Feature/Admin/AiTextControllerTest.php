...
// ─── POST /ai/suggest-seminar-name ───

describe('POST /admin/ai/suggest-seminar-name', function () {
    it('suggests a seminar name with subjects only', function () {
        actingAsAdmin();
        fakeAiResponse('Seminário sobre IA');

        $response = $this->postJson('/api/admin/ai/suggest-seminar-name', [
            'subjects' => ['Inteligência Artificial'],
        ]);

        $response->assertOk()
            ->assertJsonPath('data.text', 'Seminário sobre IA');
    });

    it('suggests a seminar name with subjects and speakers', function () {
        actingAsAdmin();
        fakeAiResponse('IA na Prática com João');

        $response = $this->postJson('/api/admin/ai/suggest-seminar-name', [
            'subjects' => ['IA', 'Machine Learning'],
            'speakers' => ['João Silva'],
        ]);

        $response->assertOk()
            ->assertJsonPath('data.text', 'IA na Prática com João');
    });
    ...
    it('validates subjects is required and min 1', function () {
        actingAsAdmin();

        $this->postJson('/api/admin/ai/suggest-seminar-name', [])
            ->assertUnprocessable()
    ...
    it('validates speakers items are strings', function () {
        actingAsAdmin();
